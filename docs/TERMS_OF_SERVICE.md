# Conditions d'Utilisation - CRM BH Company

**Dernière mise à jour : 13 février 2026**

## 1. Acceptation des Conditions

En accédant et en utilisant l'Application CRM BH Company ("l'Application"), vous acceptez d'être lié par ces Conditions d'Utilisation. Si vous n'acceptez pas ces conditions, veuillez ne pas utiliser l'Application.

## 2. Description du Service

CRM BH Company est une application de gestion de la relation client (CRM) intégrant des fonctionnalités de gestion de campagnes publicitaires Facebook Ads.

**Fonctionnalités principales :**
- Gestion des prospects et clients
- Gestion du pipeline commercial
- Création et optimisation de campagnes Facebook Ads
- Suivi des performances marketing
- Récupération automatique des leads Facebook
- Tableau de bord et rapports

## 3. Utilisation Autorisée

### 3.1 Licence d'Utilisation

Nous vous accordons une licence limitée, non exclusive, non transférable et révocable pour utiliser l'Application à des fins commerciales internes.

### 3.2 Restrictions

Vous vous engagez à NE PAS :
- Copier, modifier ou distribuer l'Application
- Tenter de décompiler ou d'extraire le code source
- Utiliser l'Application pour des activités illégales
- Créer des comptes frauduleux
- Accéder aux données d'autres utilisateurs
- Surcharger ou perturber les serveurs
- Utiliser l'Application pour spammer

## 4. Compte Utilisateur

### 4.1 Création de Compte

Pour utiliser l'Application, vous devez :
- Créer un compte avec des informations exactes
- Maintenir la confidentialité de vos identifiants
- Être responsable de toutes les activités sous votre compte
- Avoir au moins 18 ans

### 4.2 Sécurité

Vous êtes responsable de :
- La sécurité de votre mot de passe
- Toute activité effectuée depuis votre compte
- Notifier immédiatement toute utilisation non autorisée

## 5. Intégration Facebook

### 5.1 Conditions Facebook

En utilisant les fonctionnalités Facebook de l'Application, vous acceptez également :
- Les Conditions d'Utilisation de Facebook
- La Politique de la Plateforme Facebook
- Les Politiques Publicitaires de Facebook

### 5.2 Connexion Facebook

Vous devez :
- Avoir un compte Facebook Business valide
- Avoir les autorisations nécessaires sur les comptes publicitaires
- Respecter les limites de dépenses publicitaires Facebook
- Être conforme aux politiques publicitaires Facebook

### 5.3 Responsabilité du Contenu

Vous êtes seul responsable :
- Du contenu de vos campagnes publicitaires
- Du respect des lois sur la publicité
- Du ciblage approprié de vos audiences
- De la conformité RGPD pour vos publicités

## 6. Propriété Intellectuelle

### 6.1 Droits de BH Company

L'Application et tout son contenu (code, design, logos, textes) sont la propriété exclusive de BH Company et sont protégés par les lois sur la propriété intellectuelle.

### 6.2 Vos Données

Vous conservez tous les droits sur vos données (clients, prospects, campagnes). Nous ne revendiquons aucun droit de propriété sur vos données.

## 7. Données et Confidentialité

### 7.1 Collecte de Données

Nous collectons et traitons vos données conformément à notre [Politique de Confidentialité](PRIVACY_POLICY.md).

### 7.2 Données Facebook

Les données récupérées depuis Facebook via l'Application sont soumises :
- À notre Politique de Confidentialité
- À la Politique de Confidentialité de Facebook
- Au RGPD et autres réglementations applicables

## 8. Paiement et Facturation

### 8.1 Frais

L'utilisation de l'Application peut être soumise à des frais d'abonnement. Les tarifs sont disponibles sur demande.

### 8.2 Frais Facebook

Les dépenses publicitaires Facebook sont facturées directement par Facebook. BH Company n'est pas responsable de ces frais.

## 9. Disponibilité du Service

### 9.1 Garantie de Service

Nous nous efforçons de maintenir l'Application disponible 24/7, mais ne garantissons pas :
- Une disponibilité ininterrompue
- L'absence d'erreurs
- La correction immédiate des bugs

### 9.2 Maintenance

Nous pouvons effectuer des maintenances planifiées ou d'urgence avec ou sans préavis.

## 10. Limitation de Responsabilité

### 10.1 Utilisation à Vos Risques

L'Application est fournie "EN L'ÉTAT" sans garantie d'aucune sorte.

### 10.2 Exclusion de Responsabilité

BH Company ne peut être tenu responsable de :
- Pertes de données ou de profits
- Interruptions d'activité
- Performances des campagnes Facebook
- Modifications des API Facebook
- Suspension de comptes Facebook
- Violations des politiques publicitaires Facebook
- Dommages indirects ou consécutifs

### 10.3 Responsabilité Facebook

Nous ne sommes pas responsables de :
- La disponibilité de l'API Facebook
- Les changements de politique Facebook
- Les suspensions de compte Facebook
- Les erreurs dans les données Facebook

## 11. Résiliation

### 11.1 Par Vous

Vous pouvez résilier votre compte à tout moment en nous contactant.

### 11.2 Par Nous

Nous pouvons suspendre ou résilier votre compte si :
- Vous violez ces Conditions d'Utilisation
- Votre compte est inactif depuis plus de 12 mois
- Vous utilisez l'Application de manière abusive
- Requis par la loi

### 11.3 Effets de la Résiliation

En cas de résiliation :
- Votre accès à l'Application sera immédiatement révoqué
- Vos données seront conservées 30 jours puis supprimées
- Vous pouvez demander une copie de vos données avant suppression

## 12. Modifications des Conditions

Nous nous réservons le droit de modifier ces Conditions d'Utilisation à tout moment. Les modifications prendront effet dès leur publication sur cette page.

Votre utilisation continue de l'Application après modification constitue votre acceptation des nouvelles conditions.

## 13. Loi Applicable

Ces Conditions d'Utilisation sont régies par les lois françaises. Tout litige sera soumis à la juridiction exclusive des tribunaux français.

## 14. Dispositions Diverses

### 14.1 Intégralité de l'Accord

Ces Conditions d'Utilisation constituent l'intégralité de l'accord entre vous et BH Company concernant l'Application.

### 14.2 Divisibilité

Si une disposition est jugée invalide, les autres dispositions restent en vigueur.

### 14.3 Renonciation

Le fait de ne pas exercer un droit ne constitue pas une renonciation à ce droit.

## 15. Contact

Pour toute question concernant ces Conditions d'Utilisation :

**BH Company**
Email : contact@bhcompany.com
Téléphone : [Votre numéro]
Adresse : [Votre adresse]

---

**En utilisant l'Application CRM BH Company, vous acceptez ces Conditions d'Utilisation.**

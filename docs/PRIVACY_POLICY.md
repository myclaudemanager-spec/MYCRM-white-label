# Politique de Confidentialité - CRM Mon CRM

**Dernière mise à jour : 13 février 2026**

## 1. Introduction

Mon CRM ("nous", "notre", "nos") exploite l'application CRM Mon CRM (ci-après dénommée "l'Application").

Cette page vous informe de nos politiques en matière de collecte, d'utilisation et de divulgation des données personnelles lorsque vous utilisez notre Application et les choix que vous avez associés à ces données.

## 2. Collecte et Utilisation des Données

### 2.1 Données Collectées

Nous collectons plusieurs types de données différents à des fins diverses pour vous fournir et améliorer notre service :

**Données Personnelles**
- Nom et prénom
- Adresse e-mail
- Numéro de téléphone
- Adresse postale
- Informations de l'entreprise

**Données Facebook**
- Données de campagnes publicitaires Facebook (dépenses, impressions, clics)
- Informations des formulaires Lead Ads (nom, email, téléphone des prospects)
- Identifiants de campagnes et publicités

### 2.2 Utilisation des Données

Mon CRM utilise les données collectées à des fins suivantes :
- Gestion de la relation client (CRM)
- Gestion et optimisation des campagnes publicitaires Facebook
- Analyse des performances marketing
- Communication avec les prospects et clients
- Amélioration de nos services

### 2.3 Intégration Facebook

Notre Application utilise l'API Facebook Marketing pour :
- Créer et gérer des campagnes publicitaires
- Récupérer les leads générés via Facebook Lead Ads
- Analyser les performances des campagnes
- Optimiser les budgets publicitaires

**Nous NE partageons PAS vos données Facebook avec des tiers.**

## 3. Base Légale du Traitement

Le traitement de vos données personnelles est basé sur :
- Votre consentement explicite
- L'exécution d'un contrat
- Nos intérêts légitimes (amélioration du service)
- Respect des obligations légales

## 4. Conservation des Données

Nous conservons vos données personnelles uniquement aussi longtemps que nécessaire aux fins énoncées dans cette politique de confidentialité.

**Durée de conservation :**
- Données clients : Durée de la relation commerciale + 3 ans
- Données prospects : 3 ans à compter du dernier contact
- Données campagnes Facebook : 2 ans

## 5. Sécurité des Données

Nous mettons en œuvre des mesures de sécurité appropriées pour protéger vos données :
- Chiffrement des communications (HTTPS/TLS)
- Authentification sécurisée (JWT)
- Accès restreint aux données
- Sauvegardes quotidiennes automatiques
- Tokens Facebook avec permissions limitées

## 6. Vos Droits

Conformément au RGPD, vous disposez des droits suivants :
- **Droit d'accès** : Obtenir une copie de vos données
- **Droit de rectification** : Corriger vos données inexactes
- **Droit à l'effacement** : Supprimer vos données
- **Droit à la limitation** : Limiter le traitement de vos données
- **Droit à la portabilité** : Recevoir vos données dans un format structuré
- **Droit d'opposition** : S'opposer au traitement de vos données

Pour exercer ces droits, contactez-nous à : contact@bhcompany.com

## 7. Partage des Données

Nous ne vendons, n'échangeons ni ne louons vos données personnelles à des tiers.

Nous pouvons partager vos données uniquement avec :
- **Facebook** : Pour la gestion des campagnes publicitaires
- **Prestataires techniques** : Hébergement, maintenance (sous contrat de confidentialité)
- **Autorités légales** : Si requis par la loi

## 8. Cookies

Notre Application utilise des cookies pour :
- Maintenir votre session connectée
- Mémoriser vos préférences
- Analyser l'utilisation de l'Application

Vous pouvez configurer votre navigateur pour refuser les cookies, mais certaines fonctionnalités pourraient ne pas fonctionner.

## 9. Services Tiers

Notre Application intègre des services tiers :

**Facebook Marketing API**
- Fournisseur : Meta Platforms Ireland Limited
- Finalité : Gestion des campagnes publicitaires
- Politique de confidentialité : https://www.facebook.com/privacy/policy/

## 10. Modifications de la Politique

Nous pouvons mettre à jour notre politique de confidentialité périodiquement. Nous vous informerons de tout changement en publiant la nouvelle politique sur cette page et en mettant à jour la date de "Dernière mise à jour".

## 11. Contact

Pour toute question concernant cette politique de confidentialité, contactez-nous :

**Mon CRM**
Email : contact@bhcompany.com
Téléphone : [Votre numéro]
Adresse : [Votre adresse]

---

**Consentement**

En utilisant l'Application CRM Mon CRM, vous consentez à notre politique de confidentialité et acceptez ses conditions.
